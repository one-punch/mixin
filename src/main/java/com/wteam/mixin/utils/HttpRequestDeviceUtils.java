...
public class HttpRequestDeviceUtils {
     ...
     * log4j实例对象.
     */
    private static Logger logger = LogManager.getLogger(HttpRequestDeviceUtils.class);
    ...
    /**
    * 根据当前请求的特征，判断该请求是否来自手机终端，主要检测特殊的头信息，以及user-Agent这个"via"header
    ...
    public static boolean isMobileDevice(HttpServletRequest request){
        ...
        for (int i = 0; via!=null && !via.trim().equals("") && i < mobileGateWayHeaders.length; i++) {
            if(via.contains(mobileGateWayHeaders[i])){
            	logger.debug("Via mobile: " + mobileGateWayHeaders[i]);
                mobileFlag = true;
                break;
            }
        }
        for (int i = 0;!mobileFlag && userAgent!=null && !userAgent.trim().equals("") && i < mobileUserAgents.length; i++) {
            if(userAgent.contains(mobileUserAgents[i])){
            	logger.debug("user-agent mobile: " + mobileUserAgents[i]);
                mobileFlag = true;
                break;
            }
        }
        for (int i = 0; userAgent!=null && !userAgent.trim().equals("") && i < pcHeaders.length; i++) {
            if(userAgent.contains(pcHeaders[i])){
            	logger.debug("user-agent pc: " + pcHeaders[i]);
                pcFlag = true;
                break;
            }
        }
